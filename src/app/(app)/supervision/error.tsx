...
export default function SupervisionError({
  ...
  useEffect(() => {
    console.error("Supervision Command Centre error", error.digest ?? "no-digest");
  }, [error]);
    ...
    <div className="min-h-0 bg-surface-page px-6 py-6">
      <div className="mx-auto max-w-lg rounded-[12px] border border-[#e6e8e6] bg-white p-6">
        <h1 className="text-[16px] font-semibold text-[#141f19]">
          Supervision could not be loaded
        </h1>
        <p className="mt-2 text-[13px] text-[#5f6b64]">
          An error occurred while loading portfolio supervision metrics. Retry, or return to the
          dashboard.
        </p>
